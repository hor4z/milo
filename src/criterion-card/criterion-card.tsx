...
export function CriterionCard({ criterion, total, open, onToggle, onRemove, level, onLevel, meta, children, className }: {
  ...
  onRemove?: () => void
  /** En qué nivel quedó: el índice del renglón elegido. Los cuatro son excluyentes, así que es uno y no una lista. */
  level?: number
        ...
        <div id={bodyId} inert={!open} className={s.bodyInner}>
          <Card.Body className={s.levels}>
            ...
            <ul
              ...
              aria-label={onLevel ? `En qué nivel quedó ${criterion.label}` : undefined}
                ...
                const cuerpo = (
                  <>
                    <span aria-hidden className={cx(s.pick, elegido && s.met)}>
                      {elegido && <Icon name="check" weight={600} />}
                    </span>
                    <span className={cx(s.stepText, elegido && s.stepMet)}>
                      ...
                      {text}
                      {!onLevel && elegido && <span className="sr-only">, es el nivel en el que quedó</span>}
                    </span>
                  </>
                )
                return (
                  <li key={text} role={onLevel ? 'none' : undefined} className={cx(s.step, s.row, !onLevel && elegido && s.stepPicked)}>
                    {onLevel
                      ? (
                          <button
                            ...
                            onClick={() => onLevel(i)}
                            className={cx(s.option, elegido && s.selected)}
                          >
                            {cuerpo}
                          </button>
                        )
