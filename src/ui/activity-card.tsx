...
import { Chip, cx, IconButton } from './primitives'
...
export function ActivityRow({ activity }: { activity: Activity }) {
  ...
  return (
    <div className="group flex items-center gap-4 border-t border-line px-4 py-3 transition-colors first:border-t-0 hover:bg-hover">
      <span className={cx('flex size-9 shrink-0 items-center justify-center rounded-lg', `bg-tint-${activity.tint}`)}>
        <Icon name="cube" size={16} className="text-ink-muted" />
      </span>
      ...
      <Chip tint={done ? 1 : undefined}>{done ? 'completa' : `${activity.submissions}/${activity.learners}`}</Chip>
