...
describe('Steps', () => {
  it('es una lista ordenada con nombre', () => {
    render(<Steps steps={etapas} current={1} label="Etapas del proyecto" />)
  ...
  it('la etapa en curso se marca con `aria-current`, y una sola', () => {
    render(<Steps steps={etapas} current={1} label="Etapas" />)
    const marcadas = screen.getAllByRole('listitem').filter(li => li.getAttribute('aria-current') === 'step')
    expect(marcadas).toHaveLength(1)
    expect(marcadas[0]).toHaveTextContent('Definir')
  })
  ...
  it('con `onSelect` cada etapa es un botón y devuelve su índice', async () => {
    ...
    render(<Steps steps={etapas} current={1} label="Etapas" onSelect={onSelect} />)
  ...
  it('la primera etapa no cuelga una línea hacia atrás', () => {
    const { container } = render(<Steps steps={etapas} current={0} label="Etapas" />)
    const primera = container.querySelectorAll('li')[0]
    expect(primera.querySelector('[aria-hidden]')).toBeNull()
  })
})
