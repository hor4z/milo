...
import { Card, Chip, cx } from '../ui/primitives'
...
function RecipeCard({ recipe }: { recipe: (typeof recipes)[number] }) {
  return (
    <Card interactive className="flex items-start gap-3.5 p-4">
      <span className={cx('flex size-10 shrink-0 items-center justify-center rounded-xl', `bg-tint-${recipe.tint}`)}>
        <Icon name="book" size={18} className="text-ink-muted" />
      </span>
